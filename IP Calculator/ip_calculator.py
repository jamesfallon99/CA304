...
get_class_stats("192.168.10.0") #Run part 1 code here
...
def get_subnet_stats(ip_addr, subnet_mask): # Takes in a class C ip address in decimal dot notation as a string and a subnet mask
    ...
    if subnet_mask.split(".")[2] == "255": #if statement checks to see if it's a class C or B

        subnet_bits = 0 # Stores the total amount of subnet bits
        for num in binary[3]: # As class C has default subnet mask - 255.255.255.0, we can just look at the last byte to find how many subnet bits there are
            if int(num) == 1:
                subnet_bits += 1
        ...
        increment_number = 256 - int(subnet_mask_split[3]) #e.g. 256 - 192 = 64
        valid_subnets = [] #Start counting at 0 and increment by the increment number until you reach the mask, e.g. 0, 64, 128, 192
        index_of_last_octet = ip_addr.rindex(".") #As the last octet will the be the changing values, we need to know this index
        ...
        while i < len(valid_subnets): #loop through the valid subnets we found earlier
            ...
            first_addresses.append(ip_addr[:index_of_last_octet] + "." + str(subnet)) #join the valid hosts to the original ip address and append these to the first_addresses list
            i += 1
        ...
        while i < len(broadcast_addresses):#Loop through the broadcast addresses
            ...
            last_addresses.append(ip_addr[:index_of_last_octet] + "." + str(broadcast)) #join the valid hosts to the original ip address and append these to the first_addresses list
            i += 1
    ...
    #part 3 -subnet(class B) calculator
    elif subnet_mask.split(".")[2] != "255":
        subnet_bits = 0 # Stores the total amount of subnet bits
        i = 2
        while i < 4:
            for num in binary[i]: # As class C has default subnet mask - 255.255.255.0, we can just look at the last byte to find how many subnet bits there are
                if int(num) == 1:
                    subnet_bits += 1
            i += 1
        subnets = 2 ** subnet_bits
        ...
        #Valid subnets
        subnet_mask_split = subnet_mask.split(".")
        increment_number = 256 - int(subnet_mask_split[2]) #e.g. 256 - 192 = 64
        valid_subnets = [] #Start counting at 0 and increment by the increment number until you reach the mask, e.g. 0, 64, 128, 192
        index_of_last_octet = ip_addr.rindex(".")
        ...
        while total < 2:
            if ip_addr[i] == ".":
                total += 1
            i += 1
        index_of_second_last_octet = i
        for i in range(0, int(subnet_mask_split[2]) + 1, increment_number): #int(subnet_mask_split[3]) is the subnet mask byte
            valid_subnets.append(ip_addr[:index_of_second_last_octet] + str(i) + str(ip_addr[index_of_last_octet:])) #Append each valid subnet to a list
        print("Valid subnets:", valid_subnets)
        ...
        while i < len(valid_subnets): #loop through the valid subnets we found earlier
            #index_of_last_octet = valid_subnets[i].rindex(".")
            subnet_index = valid_subnets[i].rindex(".") + 1 #get the index of the subnet
            subnet = valid_subnets[i][subnet_index:] #Store the value of the start of the valid hosts for each subnet
            subnet = int(subnet) + 1 #increment by 1 to get the first value in the valid hosts
...
get_subnet_stats("172.16.0.0","255.255.192.0") #Run part 2 and 3 code here
