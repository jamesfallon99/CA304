...
def to_binary_string(ip_addr):#Helper function
...
def get_class_stats(ip_addr):
    binary_string = to_binary_string(ip_addr)
    print(binary_string)
    ...
    if binary[0:1] == "0":
        ...
        print("First address: 0.0.0.0") #Hard coded first and last addresses because we know the values and they will never change
    ...
    """
get_class_stats("198.252.255.0") #Run part 1 code here





def to_decimal_dot(ip_addr_list):#Helper function
...
def get_unmasked_bits(index, binary_list):#Helper function to get the unmaked bits
    #Takes an index and an array of the subnet mask converted to binary
    #counts up all the 0s as these are the unmasked bits
    #returns the unmasked bits
    unmasked_bits = 0
    while index < 4:#The binary list will be divided up into 4 parts, loop through to the end of this list
        for num in binary_list[index]:
            if int(num) == 0: # Chcek if the number is a 0
                unmasked_bits += 1 #If it is, increment our unmasked bits variable
        index += 1
    return unmasked_bits
...
def get_subnet_stats(ip_addr, subnet_mask): # Takes in a class C or class B or class A ip address in decimal dot notation as a string and a subnet mask
    #subnetting-dividing our network up into isolated components
    #Reduces network traffic, optimizes network performace and simplies management
    #subnet mask is used to tell the difference between the network part and the host part of an address
    #When converted to binary, all the network bits are represented as 1s and all the host bits are represented as 0s

    #The ip address in CIDR notation(Classless Inter Domain Routing):
    ...
    #Find how many subnet bits and plug into formula - 2 ** subnet bits
    ...
    if subnet_mask.split(".")[2] == "255": #check if this is a class C
        ...
        i = 1#If it is, let i = 1
    elif subnet_mask.split(".")[2] != "255": #Check if this is a class B
        i = 2 #If it is, let i = 2
        #As class C has default subnet mask - 255.255.255.0, assuming we can just look at the last byte to find how many subnet bits there are.
        #As class B has default subnet mask - 255.255.0.0, assuming we can look at the second last byte to find out how many subnet bits there are.
        #As class A has default subnet mask - 255.0.0.0, assuming we can look at the second byte to find out how many subnet bits there are.
    for num in binary[i]:
        if int(num) == 1:
            subnet_bits += 1
    ...
    #Check how many hosts per subne, and sub into formula -(2 ** y)-2 where y is the number of unmasked bits(0s)
    unmasked_bits = 0
    if subnet_mask.split(".")[1] != "255": #Check it's a class A
        i = 1
        unmasked_bits = get_unmasked_bits(i, binary) #Use the helper function "get_unmasked_bits" from above to get the unmasked bits
    ...
    #Valid subnets
    #Have to see where the subnets start and end
    #In order to see this, need to get the increment number(256-subnet mask)
    ...
    if subnet_mask.split(".")[1] != "255": #If it's a class A address
        ip_addr_split = ip_addr.split(".") #Split the ip address
        index_of_second_octet = ip_addr.index(".")#Get the index of the second octet
        third_octet = str(ip_addr_split[2])#Get the third octet
        i = 1
    ...
    #broadcast address of eaqch subnet
...
#get_subnet_stats("192.168.10.0","255.255.255.192") #Run part 2 and 3 code here. Part 3 can be run with: get_subnet_stats("172.16.0.0","255.255.192.0"). Also run extra marks(class A) here
